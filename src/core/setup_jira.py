...
MCP_CONFIG_PATH = "mcp_config_jira.json"
_PLACEHOLDER = "YOUR_JIRA_MCP_URL"


def _load_mcp_config() -> str:
    """Read Jira MCP config and substitute the MCP URL from the environment."""
    jira_mcp_url = os.getenv("JIRA_MCP_URL")
    if not jira_mcp_url:
        raise ValueError("JIRA_MCP_URL must be set in .env")
    ...
    return content.replace(_PLACEHOLDER, jira_mcp_url)


def create_collection(client: H2OGPTE) -> str:
    """Create a new H2OGPTE collection for the Jira action agent."""
    collection_id = client.create_collection(
        name="Jira Action Agent",
        description="Jira Action Agent with Jira Remote MCP Tool Capabilities",
    )
...
def upload_and_ingest_mcp_config(client: H2OGPTE, collection_id: str) -> str:
    """Upload and ingest the Jira MCP config file into the collection."""
...
def register_mcp_tool(client: H2OGPTE) -> list:
    """Register the Jira MCP tool with H2OGPTE."""
...
def setup_agent_keys(client: H2OGPTE) -> None:
    """Ensure agent keys for Jira MCP env vars exist, reusing or creating as needed."""
    required_keys = {
